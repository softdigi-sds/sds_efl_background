...
class SmartData
{
    ...
    static public function post_data($name, $type)
    {
        ...
        switch ($type) {
            ...
            case "FLOAT":
                return self::post_float_data($name);
            case "EMAIL":
                return self::post_email_data($name);
            case "ARRAY":
                return self::post_array_data($name);
            case "ARRAY_TO_STRING":
                return self::post_array_to_string_data($name);
            case "SELECT_VALUE":
                return self::post_select_value($name);
            case "DATE":
                return self::post_date_data($name);
            case "STRING":
            default:
                return self::post_string_data($name);
        }
    }
