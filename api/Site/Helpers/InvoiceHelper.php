...
class InvoiceHelper extends BaseHelper
{
    ...
    public function getOneData($id)
    {
        ...
        $select = [
            ...
            DATE_FORMAT(t12.bill_start_date,'%d-%m-%Y') as start_date,
             DATE_FORMAT(t12.bill_end_date,'%d-%m-%Y') as end_date",
            "DATE_FORMAT(t12.invoice_date,'%d/%m/%Y') as invoice_date,
            DATE_FORMAT(t12.due_date,'%d/%m/%Y') as due_date",
    ...
    public function getUnitValues($rates, $meter_id)
    {
        ...
        foreach ($rates as $obj) {
            if ($obj->sd_hsn_id["value"] == 3 && $meter_id == 1) {
                ...
                $unit_price = $obj->price;
                $hsn_id = 5;
            } else if ($obj->sd_hsn_id["value"] == 7 && $meter_id == 5) {
                $unit_price = $obj->price;
                $hsn_id = 7;
            }
        }
